<?php
    if(isset($_POST))
    {
        ...
        $res = array();
        $res['text'] = "Error";
        $acc = $con->query("select account from accounts inner join accounts_log on accounts.id = accounts_log.account_id where accounts_log.id = '$acc_id' and workspace_id = '$wid'")->fetch_assoc()['account'];
