...
foreach($result as $row)
{
 ...
 $sub_array[] = $row['con'];
 $sub_array[] = $row['adate'];
 $sub_array[] = "<a href='../workspace.php?cid=".trim($row['aid'])."'><i class='fas fa-edit editClient' id='".$row['aid']."'></i></a>";
 $data[] = $sub_array;
}
function get_all_data($con)
{
 $query = "SELECT count(a.id) total FROM client a where a.active = 1 ";
