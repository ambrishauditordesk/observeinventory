...
foreach (($_POST['pname']) as $personname) {
    $pname[$i++] = trim($personname);
}
...
foreach (($_POST['phone']) as $phoneno) {
    $phone[$i++] = trim($phoneno);
}
...
if($uploadOk) {
    ...
    $cid = $con->insert_id;
    for($i=0;$i<$count;$i++){
        $con->query("insert into contact(client_id,pname,email,phone,designation) values('$cid','$pname[$i]','$email[$i]','$phone[$i]','$desig[$i]')");
    }
    echo "<script>
