...
function getProgramStatus($id, $wid){
    ...
    if($id != 12){
        ...
        $data['totalCount'] += (int)$con->query("SELECT count(id) total FROM workspace_log where program_id >= 35 and program_id <= 46 and workspace_id = $wid")->fetch_assoc()['total'];
        $data['statusCount'] += (int)$con->query("SELECT count(id) total FROM workspace_log where program_id >= 35 and program_id <= 46 and workspace_id = $wid and amount != ''")->fetch_assoc()['total'];
